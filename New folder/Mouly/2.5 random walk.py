...
def get_probabilities():
    while True:
        try:
            ...
            backward_prob = float(input("Enter probability for moving Backward: "))

            total = forward_prob + left_prob + right_prob + backward_prob

            if abs(total - 1.0) < 0.01:
                return [forward_prob, left_prob, right_prob, backward_prob]
            else:
...
for step in range(1, steps + 1):
    ...
    if rand_num < probabilities[0]:  # Move Forward
        ...
        direction = "Left"
        x -= 1
    elif rand_num < probabilities[0] + probabilities[1] + probabilities[2]:  # Move Right
        direction = "Right"
        x += 1
    else:  # Move Backward
        direction = "Backward"
